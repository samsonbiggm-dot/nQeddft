...
class QEDPhonon:
    ...
    def cavity_shift(self, verbose: bool = True) -> dict:
        ...
        fix: 参考态使用与 self.mf 相同的 SCF 类型（RKS 或 UKS），
             避免对开壳层体系用 QEDRKS 计算参考态导致错误。
        ...
        # fix: 参考态 SCF 类型必须与 self.mf 一致
        MFClass = QEDUKS if isinstance(self.mf, QEDUKS) else QEDRKS
        mf0 = MFClass(mol, cav0)
        mf0.xc      = self.mf.xc
        mf0.verbose = 0
        # 继承 SCF 收敛设置
        for attr in ('level_shift', 'max_cycle', 'init_guess', 'conv_tol'):
            if hasattr(self.mf, attr):
                setattr(mf0, attr, getattr(self.mf, attr))
        if verbose:
            print(f"计算自由空间振动频率（lambda=0 参考态，{MFClass.__name__}）...")
        mf0.kernel()
    ...
    def _gradient(self, mf, coords: np.ndarray) -> np.ndarray:
        ...
        fix: 自动根据 mf 类型选择 QEDGradients（RKS）或
             QEDUKSGradients（UKS），避免 UKS 体系出现
             IndexError（mo_occ 维度不匹配）。
        """
        ...
        mf.verbose = 0
        mf.kernel()
        GradClass   = QEDUKSGradients if isinstance(mf, QEDUKS) else QEDGradients
        g_obj       = GradClass(mf)
        g_obj.verbose = 0
        return g_obj.kernel()
