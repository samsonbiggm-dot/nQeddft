...
class PauliFierzCorrection:
    ...
    def get_vqed(self, dm: np.ndarray) -> np.ndarray:
        ...
        推导（偶极规范，相干态近似）：
          E_QED_total = <λ·d_total>² · (1/2 - 1/(2ω))
          其中 d_total = d_elec + d_nuc，d_nuc = Σ_I Z_I R_I 为常数。

          对密度矩阵 P 变分：
            δE_QED/δP_μν = (μ_e + μ_n) · (1 - 1/ω) · (λ·d)_μν

          其中：
            μ_e = Tr[(λ·d)·P]         ← 电子偶极（P 的函数）
            μ_n = λ · Σ_I Z_I R_I     ← 核偶极（几何固定时为常数）

          **必须包含 μ_n**：核偶极改变相干态振幅 α = (μ_e+μ_n)/√(2ω)，
          进而影响腔场对电子的反作用。若仅用 μ_e，在非质心坐标系下
          Fock 矩阵与总能量不自洽，解析梯度出现系统误差。

        注意：不存在额外的 V_K（exchange-like）项。
        V_DSE 的 Fock 导数对 Hermitian P 只有一项：δ<λ·d_total>²/δP_μν
        = 2<λ·d_total>·(λ·d)_μν，乘以 1/2 后即 <λ·d_total>·(λ·d)_μν。
        """
        ...
        for mode in self.cavity.modes:
            ...
            ld_elec = float(np.einsum('pq,qp->', ld, dm))       # <λ·d_elec>
            ld_nuc  = float(np.dot(lam, self.dip_ints.nuclear_dipole()))  # λ·d_nuc
            ld_mean = ld_elec + ld_nuc                           # <λ·d_total>

            # V_DSE = <λ·d_total> · (λ·d)_μν
            # 来自 E_DSE = (1/2)<λ·d_total>²，对 P 的 Fock 导数
            vqed += ld_mean * ld

            # V_bilinear = -(1/ω) · <λ·d_total> · (λ·d)_μν
            # 相干态近似：α = <λ·d_total>/√(2ω)，V_bil = -√(ω/2)·2α·ld
